/**
 * Account Health Indicator Component
 * Displays account health status with visual indicators
 */

interface AccountHealthIndicatorProps {
  status: 'healthy' | 'warning' | 'critical' | 'unknown';
  label?: string;
  className?: string;
}

export default function AccountHealthIndicator({ 
  status, 
  label,
  className = '' 
}: AccountHealthIndicatorProps) {
  const statusConfig = {
    healthy: {
      color: 'text-brand-success-500',
      bg: 'bg-brand-success-500/20',
      border: 'border-brand-success-500/30',
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
        </svg>
      ),
      text: 'Healthy'
    },
    warning: {
      color: 'text-brand-warning-500',
      bg: 'bg-brand-warning-500/20',
      border: 'border-brand-warning-500/30',
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
        </svg>
      ),
      text: 'Warning'
    },
    critical: {
      color: 'text-brand-error-500',
      bg: 'bg-brand-error-500/20',
      border: 'border-brand-error-500/30',
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
        </svg>
      ),
      text: 'Critical'
    },
    unknown: {
      color: 'text-dark-text-tertiary',
      bg: 'bg-dark-bg-tertiary/50',
      border: 'border-dark-border-primary',
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
        </svg>
      ),
      text: 'Unknown'
    }
  };

  const config = statusConfig[status];

  return (
    <div 
      className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg border ${config.bg} ${config.border} ${config.color} ${className}`}
      role="status"
      aria-label={`Account status: ${config.text}`}
    >
      <span className={config.color}>
        {config.icon}
      </span>
      <span className="text-sm font-medium">
        {label || config.text}
      </span>
    </div>
  );
}
